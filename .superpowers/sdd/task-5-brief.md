...
**Files:**
- Create: `wp-security-hub/src/config/settings.ts`
- Create: `wp-security-hub/tests/storage/site-repo.test.ts`

**Interfaces:**
- Consumes: none (reads from filesystem)
- Produces: `Settings`, `loadSettings(rootDir: string): Settings`, `saveSettings(rootDir: string, settings: Settings): void`
...
git commit -m "feat: add settings manager with YAML config support"
```
