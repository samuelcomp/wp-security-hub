 Reconnaissance Module (Round 1)

**Files:**
- Create: `wp-security-hub/src/core/modules/recon.ts`
- Create: `wp-security-hub/tests/core/modules/recon.test.ts`

**Interfaces:**
- Consumes: `RemoteConnection` from `connectors/types.ts`, `AgentMemory` from `engine/types.ts`, `Finding`, `CreateFindingInput` from `finding-repo.ts`
- Produces: `ReconModule.run(connection, siteId, scanId, vaultDir): Promise<{memory: AgentMemory, findings: Finding[]}>`
...
```bash
git add src/core/modules/recon.ts
git commit -m "feat: add reconnaissance module for Round 1 investigation"
```
