...
**Files:**
- Create: `wp-security-hub/src/fix/fix-engine.ts`
- Create: `wp-security-hub/tests/fix/fix-engine.test.ts`

**Interfaces:**
- Produces: `FixEngine.executeApproved(siteId, scanId, connection): Promise<FixRecord[]>`

- [ ] **Step 1: Write fix-engine.ts**
...
```bash
git add src/fix/fix-engine.ts
git commit -m "feat: add fix engine for executing approved fixes"
```
