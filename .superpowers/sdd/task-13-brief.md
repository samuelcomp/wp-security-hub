 External Intel Module (Round 2)

**Files:**
- Create: `wp-security-hub/src/core/modules/external-intel.ts`

**Interfaces:**
- Consumes: `AgentMemory`, `Settings` from `config/settings.ts`
- Produces: `ExternalIntel.run(memory, settings, siteId, scanId): Promise<CreateFindingInput[]>`

- [ ] **Step 1: Write external-intel.ts**
...
```bash
git add src/core/modules/external-intel.ts
git commit -m "feat: add external intelligence module (WPScan + Sucuri)"
```
