...
**Files:**
- Create: `wp-security-hub/src/core/connectors/types.ts`
- Create: `wp-security-hub/src/core/connectors/ssh.ts`

**Interfaces:**
- Consumes: `SiteCredentials` and `SshCredentials` from `types.ts`
- Produces: `SshConnection` with `connect()`, `exec(command: string): Promise<string>`, `downloadFile(remotePath: string, localPath: string): Promise<void>`, `disconnect()`

- [ ] **Step 1: Write connectors/types.ts**
...
```bash
git add src/core/connectors/types.ts src/core/connectors/ssh.ts
git commit -m "feat: add SSH connector with exec, file download/upload, and SFTP support"
```
